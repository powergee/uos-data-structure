...
int main(int argc, char* argv[])
{
    ...
    {
        ++total_count;
        found = search(&fifolist, blkno);

        // 캐시에서 히트되지 않았을 경우
        if (found == NULL)
        {
            // 캐시에서 히트되지 않았다면 캐시의 빈 노드에 값을 할당하거나,
            // 캐시에 더이상 공간이 없다면 FIFO 교체 정책에 따라
            // 가장 처음에 넣은 값을 제거해야 함.

            // 캐시에 더이상 공간이 없는 경우,
            // 가장 처음에 넣은 값을 제거하고 그 자리에 지금의 값을 넣음.
            if (is_empty(&free_node_list))
            {
                empty_buffer = delete(&fifolist, fifolist.prev);
                empty_buffer->blkno = blkno;
                insert(&fifolist, empty_buffer);
            }
            // 캐시에 빈 공간이 있는 경우,
